...
export default function Booking() {
  ...
  useEffect(() => {
    ...
    if (query.get('payment') === 'success') { setTrackingBookingId(localStorage.getItem('viso:lastBookingId') || ''); setStep(5) }
  }, [])
  ...
  useEffect(() => {
    if (!appointment.date || !selectedIds.length) { setAvailability([]); return }
    let cancelled = false
    setAvailabilityLoading(true)
    const url = import.meta.env.VITE_SUPABASE_URL
    const anon = import.meta.env.VITE_SUPABASE_ANON_KEY
    fetch(`${url}/functions/v1/get-availability`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', apikey: anon },
      body: JSON.stringify({ date: appointment.date, serviceIds: selectedIds }),
    }).then(r => r.json()).then(data => { if (!cancelled) { setAvailability(Array.isArray(data.slots) ? data.slots : []); if (!data.slots?.some(x => x.time === appointment.time)) setAppointment(v => ({ ...v, time: '' })) } }).catch(() => { if (!cancelled) setAvailability([]) }).finally(() => { if (!cancelled) setAvailabilityLoading(false) })
    return () => { cancelled = true }
  }, [appointment.date, selectedIds.join(',')])
  ...
  async function completePayment() {
    ...
    try {
      ...
      const response = await fetch(`${url}/functions/v1/create-checkout-session`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', apikey: anon, Authorization: `Bearer ${(() => { try { return JSON.parse(localStorage.getItem('viso:customer-session') || 'null')?.access_token || anon } catch { return anon } })()}` },
      ...
      if (result.bookingId) localStorage.setItem('viso:lastBookingId', result.bookingId)
          ...
          <section className="booking-card rounded-[2rem] border border-line bg-white p-5 shadow-[0_18px_60px_rgba(35,35,55,.06)] md:p-8">
            ...
            {step === 4 && (
              <>
                ...
                <p className="mt-3 text-sm leading-6 text-muted">Choose a day, then select from times Viso can actually cover. If the schedule is full, choose another time or date.</p>
                <div className="mt-7 grid gap-4 sm:grid-cols-2">
                  ...
                  <label className="text-sm font-medium">Available time<select value={appointment.time} onChange={update(setAppointment, 'time')} className="field mt-2" disabled={!appointment.date || availabilityLoading}><option value="">{availabilityLoading ? 'Checking availability…' : !appointment.date ? 'Choose a date first' : availability.length ? 'Choose an available time' : 'No times available'}</option>{availability.map(slot => <option key={slot.time} value={slot.time}>{slot.time} · {slot.technicianCount} technician{slot.technicianCount===1?'':'s'} available</option>)}</select></label>
                </div>
            ...
            {step === 5 && (
              <>
                ...
                {trackingBookingId && <div className="mt-6 rounded-2xl border border-blue/15 bg-blue/[.035] p-5">
                  <p className="text-xs font-semibold uppercase tracking-[.2em] text-blue">Live service tracking</p>
                  <h3 className="mt-2 text-xl font-display">Follow your technician.</h3>
                  <p className="mt-2 text-sm leading-6 text-muted">Once your payment is confirmed and your technician starts sharing location, the map will update with their latest verified position.</p>
                  <div className="mt-5"><LiveTracking bookingId={trackingBookingId} customerEmail={contact.email} destination={location} /></div>
                </div>}
